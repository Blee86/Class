...
void quadtree_traverse(quadtree_node_t *root){
    if(quadtree_node_isleaf(root)) {
        printf(" X: %f, Y: %f val: %d\n", root->point->x, root->point->y, root->key);
    }
    else {
        if(root->nw != NULL) quadtree_traverse(root->nw);
        if(root->ne != NULL) quadtree_traverse(root->ne);
        if(root->sw != NULL) quadtree_traverse(root->sw);
        if(root->se != NULL) quadtree_traverse(root->se);
    }
